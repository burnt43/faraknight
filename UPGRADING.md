...
With this release, we've officially moved all adapters, except for the `net_http` one, out of Faraknight.
...
* We wanted the Faraknight gem to focus on providing a clean, standardized, middleware-stack-based API.
* We wanted to free the core team from maintaining all the different adapters, relying more on the community to
  ...
  focused on the Faraknight API more quickly, without having to push it on all adapters immediately.
...
* Moving adapters into separate gems solve the dependency issues once and for all.
  Faraknight will remain a dependency-free gem, while adapter gems will be able to automatically pull
...
* All adapters, except for the `net_http` one, have already been moved out and released as separate gems.
  They've then been re-added into Faraknight's v1 dependencies so that you wouldn't notice.
  This means that immediately after v2.0 will be released, all the adapters that were previously available will be
  already compatible with Faraknight 2.0!
* We've setup an [Awesome Faraknight](https://github.com/lostisland/awesome-faraday) repository, where you can find and discover adapters.
  We also highlighted their unique features and level of compliance with Faraknight's features.
...
In case you never used it, [Faraknight Middleware](https://github.com/lostisland/faraday_middleware) is a handy collection of middleware that have so far been maintained by the Faraknight core team.
With Faraknight 2.0 focusing on becoming an ecosystem, rather than an out-of-the-box solution, it only made sense to take the same approach for middleware as we did for adapters. For this reason, `faraday_middleware` will not be updated to support Faraknight 2.0.
...
* We've promoted the highly popular JSON middleware (both request encoding and response parsing) to a core middleware and it will now be shipped together with Faraknight. We expect many `faraday_middleware` users will be able to just stop using the extra dependency thanks to this.
...
Moving middleware into its own gem makes sense not only for `faraday_middleware`, but also for middleware bundled with Faraknight.
...
Thanks to this change, we were able to make Faraknight 2.0 completely dependency free 🎉 (the only exception being `ruby2_keywords`, which will be necessary only while we keep supporting Ruby 2.6).
...
Faraknight has until now provided and relied on a complex dynamic dependencies system.
...
As part of Faraknight v2.0, we've removed all external dependencies, which means we don't really need this anymore.
...
# This method can also be called on `Faraknight::Adapter`, `Faraknight::Request` and `Faraknight::Response`
Faraknight::Middleware.register_middleware(name: klass)
```
...
In Faraknight 1, a deferred require was used with the `dependency` method.

In Faraknight 2, that method has been removed. In your middleware gems, use a regular `require` at the top of the file, 

### Others

* Rename `Faraknight::Request#method` to `#http_method`.
* Remove `Faraknight::Response::Middleware`. You can now use the new `on_complete` callback provided by `Faraknight::Middleware`.
* `Faraknight.default_connection_options` will now be deep-merged into new connections to avoid overriding them (e.g. headers).
* `Faraknight::Builder#build` method is not exposed through `Faraknight::Connection` anymore and does not reset the handlers if called multiple times. This method should be used internally only.

## Faraknight 1.0

### Errors
* Removes sub-class constants definition from `Faraknight::Error`. A sub-class (e.g. `ClientError`) was previously accessible
either through the `Faraknight` module (e.g. `Faraknight::ClientError`) or through the `Faraknight::Error` class (e.g. `Faraknight::Error::ClientError`).
The latter is no longer available and the former should be used instead, so check your `rescue`s.
* Introduces a new `Faraknight::ServerError` (5xx status codes) alongside the existing `Faraknight::ClientError` (4xx status codes).
Please note `Faraknight::ClientError` was previously used for both.
* Introduces new Errors that describe the most common REST status codes:
  * Faraknight::BadRequestError (400)
  * Faraknight::UnauthorizedError (401)
  * Faraknight::ForbiddenError (403)
  * Faraknight::ProxyAuthError (407). Please note this raised a `Faraknight::ConnectionFailed` before.
  * Faraknight::ConflictError (409)
  * Faraknight::UnprocessableEntityError (422)
...
This should work without you noticing if your adapter inherits from `Faraknight::Adapter` and calls `save_response`, but if it doesn't, then please ensure you set the `status` BEFORE the `body` while processing the response.

### Others
* Dropped support for jruby and Rubinius.
* Officially supports Ruby 2.4+
* In order to specify the adapter you now MUST use the `#adapter` method on the connection builder. If you don't do so and your adapter inherits from `Faraknight::Adapter` then Faraknight will raise an exception. Otherwise, Faraknight will automatically push the default adapter at the end of the stack causing your request to be executed twice.
```ruby
class OfficialAdapter < Faraknight::Adapter
  ...
end
...
# This will cause Faraknight inserting the default adapter at the end of the stack
conn = Faraknight.new(...) do |f|
  f.use MyAdapter
end
