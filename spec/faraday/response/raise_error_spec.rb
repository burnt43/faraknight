...
RSpec.describe Faraknight::Response::RaiseError do
  let(:conn) do
    Faraknight.new do |b|
  ...
  it 'raises Faraknight::BadRequestError for 400 responses' do
    expect { conn.get('bad-request') }.to raise_error(Faraknight::BadRequestError) do |ex|
  ...
  it 'raises Faraknight::UnauthorizedError for 401 responses' do
    expect { conn.get('unauthorized') }.to raise_error(Faraknight::UnauthorizedError) do |ex|
  ...
  it 'raises Faraknight::ForbiddenError for 403 responses' do
    expect { conn.get('forbidden') }.to raise_error(Faraknight::ForbiddenError) do |ex|
  ...
  it 'raises Faraknight::ResourceNotFound for 404 responses' do
    expect { conn.get('not-found') }.to raise_error(Faraknight::ResourceNotFound) do |ex|
  ...
  it 'raises Faraknight::ProxyAuthError for 407 responses' do
    expect { conn.get('proxy-error') }.to raise_error(Faraknight::ProxyAuthError) do |ex|
  ...
  it 'raises Faraknight::RequestTimeoutError for 408 responses' do
    expect { conn.get('request-timeout') }.to raise_error(Faraknight::RequestTimeoutError) do |ex|
  ...
  it 'raises Faraknight::ConflictError for 409 responses' do
    expect { conn.get('conflict') }.to raise_error(Faraknight::ConflictError) do |ex|
  ...
  it 'raises legacy Faraknight::UnprocessableEntityError for 422 responses' do
    expect { conn.get('unprocessable-content') }.to raise_error(Faraknight::UnprocessableEntityError) do |ex|
  ...
  it 'raises Faraknight::UnprocessableContentError for 422 responses' do
    expect { conn.get('unprocessable-content') }.to raise_error(Faraknight::UnprocessableContentError) do |ex|
      expect(ex.message).to eq('the server responded with status 422 for GET http:/unprocessable-content')
      expect(ex.response[:headers]['X-Reason']).to eq('because')
      expect(ex.response[:status]).to eq(422)
      expect(ex.response_status).to eq(422)
  ...
  it 'raises Faraknight::TooManyRequestsError for 429 responses' do
    expect { conn.get('too-many-requests') }.to raise_error(Faraknight::TooManyRequestsError) do |ex|
  ...
  it 'raises Faraknight::NilStatusError for nil status in response' do
    expect { conn.get('nil-status') }.to raise_error(Faraknight::NilStatusError) do |ex|
  ...
  it 'raises Faraknight::ClientError for other 4xx responses' do
    expect { conn.get('4xx') }.to raise_error(Faraknight::ClientError) do |ex|
  ...
  it 'raises Faraknight::ServerError for 500 responses' do
    expect { conn.get('server-error') }.to raise_error(Faraknight::ServerError) do |ex|
  ...
  describe 'request info' do
    ...
    it 'returns the request info in the exception' do
      expect { perform_request }.to raise_error(Faraknight::BadRequestError) do |ex|
    ...
    describe 'DEFAULT_OPTION: include_request' do
      before(:each) do
        Faraknight::Response::RaiseError.instance_variable_set(:@default_options, nil)
  ...
  describe 'allowing certain status codes' do
    ...
    it 'raises an error for status codes that are not explicitly allowed' do
      expect { conn.get('bad-request') }.to raise_error(Faraknight::BadRequestError)
    end
